...
Steve Bromley     
10-30-19
Peak finding code for CCD data
"""
import os
import numpy as np
import scipy
...
cwd = os.getcwd()
data_path = 'C:/Users/steve_000/Desktop/filtered_data'
os.chdir(data_path)   
...
for data_in in manual_input:
    ...
    for frame in range(frame1,frame2):#range(frame1,frame2):
        ...
        indexes = peakutils.peak.indexes(pin[frame,:], thres=baseline + 100,\
    ...
    num_Bins = 764
    #Mean peak positions:
    peaks_binned = scipy.stats.binned_statistic(output_2d[:,0],output_2d[:,0], statistic='mean', \
                                                bins = num_Bins, range=(min_lambda,max_lambda))
    #Number of counts (intensity) in each bin
    counts_binned = scipy.stats.binned_statistic(output_2d[:,0],output_2d[:,1], statistic='sum', \
                                                 bins = num_Bins , range=(min_lambda,max_lambda))
    #Number of peaks used to calculate the positions and counts in each bin
    pks_per_bin = scipy.stats.binned_statistic(output_2d[:,0],output_2d[:,1], statistic='count', \ 
                                               bins = num_Bins , range=(min_lambda,max_lambda))
    #Calculate FINAL stdev (add stdev in quadrature):
    peaks_stdev = scipy.stats.binned_statistic(output_2d[:,0],output_2d[:,0], statistic=stdev,\
                                               bins = num_Bins, range= (min_lambda,max_lambda))
    os.chdir(cwd)
...
testing2 = scipy.stats.binned_statistic(all_peak_data_sorted[:,0],all_peak_data_sorted[:,0],\
                                        statistic='mean',  bins = num_Bins, range= \
                                        (global_min_lambda,global_max_lambda))
testing_count2 = scipy.stats.binned_statistic(all_peak_data_sorted[:,0],\
        all_peak_data_sorted[:,1], statistic='sum',\
        bins = num_Bins, range = (global_min_lambda, global_max_lambda))
testing2_pks_per_bin = scipy.stats.binned_statistic(all_peak_data_sorted[:,0],\
        all_peak_data_sorted[:,2],statistic='sum', bins = num_Bins , \
        range=(global_min_lambda,global_max_lambda))
testing_stdev = scipy.stats.binned_statistic(all_peak_data_sorted[:,0],\
        all_peak_data_sorted[:,3], statistic=quad_std, bins = num_Bins,\
        range= (global_min_lambda,global_max_lambda))
#%%
...
np.savetxt('final_output.txt',final,fmt='%.7f',delimiter=',')
#%%
#Remove the temporary .csv's made from peak-finding of each input file
